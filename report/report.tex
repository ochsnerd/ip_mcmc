...
the standard random walk (see also \hyperref[sec:org608c9e3]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:org7f9437a}
\subsection{Framework/Package Structure}
\label{sec:org15d1f7b}
...
\begin{enumerate}
\item AnalyticPotential
\label{sec:org7250a12}
...
see \ref{sec:org408c7b6}
\item EvolutionPotential
\label{sec:orgaa9f823}
...
\label{sec:org64565e2}

Based on formula (1.2) in \cite{cotter_mcmc_2013}:
$$a = \text{min}\{1, \exp(I(u) - I(v))\},$$ with
$$I(u) = \Phi(u) + \frac{1}{2}\norm{\C^{-1/2}u}^2$$.

See also \ref{sec:org820473e}.

\item pCNAccepter
\label{sec:org3ce07bd}

Works together with the \hyperref[sec:org8526656]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\item CountedAccepter
\label{sec:org43c52ea}
...
\subsection{Results}
\label{sec:org222a46d}
\subsubsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org122b94e}
\begin{enumerate}
\item Setup
\label{sec:org3bc8385}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org408c7b6]{before}, and
compared the following samplers:
\begin{itemize}
\item (1) \hyperref[sec:org1ff7505]{\texttt{StandardRWProposer}} (\(\delta=0.25\)) + \hyperref[sec:org356d302]{\texttt{AnalyticAccepter}}
\item (2) \hyperref[sec:org1ff7505]{\texttt{StandardRWProposer}} (\(\delta=0.25\)) + \hyperref[sec:org64565e2]{\texttt{StandardRWAccepter}}
\item (3) \hyperref[sec:org8526656]{\texttt{pCNProposer}} (\(\beta=0.25\)) + \hyperref[sec:org3ce07bd]{\texttt{pCNAccepter}}
\end{itemize}

The code is in \href{scripts/analytic.py}{\texttt{analytic.py}}.

\item Result
\label{sec:org0627f42}
...
\subsubsection{Geophysics example: Lorenz-96 model}
\label{sec:org3956c22}
\begin{enumerate}
\item Based on:
\label{sec:orgf32f426}
