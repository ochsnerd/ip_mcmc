...
the standard random walk (see also \hyperref[sec:org5cfd81b]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:orga6ae41e}

Works together with the \hyperref[sec:orgd2e3d41]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:org00894bd}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:orgc9e5245}
...
\section{Results}
\label{sec:orga7c5c1c}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org3aa02b2}
\subsubsection{Setup}
\label{sec:org9304b09}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org0fbc8bf]{before}, and
...
\label{sec:orgeb11a2a}
\subsubsection{Model}
\label{sec:org222dc2b}
\paragraph{Based on:}
\label{sec:orgace2e45}
...
\item Due to the ergodic properties of \(\mathcal{M}\) \ref{sec:org2d5eb3a} , it doesn't really matter what \(z_0\) is
...
\paragraph{Result}
\label{sec:org96e6748}

\subparagraph{Density plot for posterior}
\label{sec:org414a114}
...
\label{sec:org69e3b07}
\subsubsection{Model}
\label{sec:org019b216}
\paragraph{Burgers' Equation}
\label{sec:orgf1656e8}
...
\label{sec:org351ba67}

Blabla
\subsubsection{MCMC}
\label{sec:org684166c}

\paragraph{Hyperparameter tuning}
\label{sec:org5d85558}
...
\subparagraph{Noise}
\label{sec:org3b75106}

\(\eta \sim \N{0}{\gamma^2 \I_5}\) with \(\gamma = 0.05\).

\subparagraph{Prior}
\label{sec:org826ebd2}
