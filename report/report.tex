...
the standard random walk (see also \hyperref[sec:org7ee1131]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:org7e224ad}

Works together with the \hyperref[sec:orgf1b8e17]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:orgb64412a}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:orgf4134cb}
...
\section{Results}
\label{sec:org2c6aa25}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:orgcb992e2}
\subsubsection{Setup}
\label{sec:org5677c7c}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org55f7ed4]{before}, and
...
\label{sec:orgd1ed577}

\subsubsection{Based on:}
\label{sec:orgaa0ae23}
