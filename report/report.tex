...
\hypersetup{
 pdfauthor={David Ochsner \\ Supervisor: Siddhartha Mishra},
...
the standard random walk (see also \hyperref[sec:org0e8e01e]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:orgd28e1b5}

Works together with the \hyperref[sec:orgef1b62d]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:orgab7af9f}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:orgc870fc1}
...
\section{Results}
\label{sec:orgeca3936}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org09cc933}
\subsubsection{Setup}
\label{sec:orge54ba7a}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:orgf6b19e3]{before}, and
...
\paragraph{Result}
\label{sec:org5f78f92}

\subparagraph{Density plot for posterior}
\label{sec:org8894d45}
...
\label{sec:orgb39dc2c}
\subsubsection{Model}
\label{sec:org4d06990}
\paragraph{Burgers' Equation}
\label{sec:org71a4c45}
...
\label{sec:orge87772a}

Blabla
\subsubsection{MCMC}
\label{sec:org393968d}

\paragraph{Hyperparameter tuning}
\label{sec:orgde3df5e}
...
\label{sec:org79cd44f}
\begin{enumerate}
\item Convergence over chain length
\label{sec:org40f2e0c}
...
\begin{center}
\includegraphics[width=.9\linewidth]{./Selbststandigkeitserklarung.png}
\end{center}}
