...
the standard random walk (see also \hyperref[sec:org1e87b20]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:orgc96e13f}

Works together with the \hyperref[sec:org6a031e1]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:org7351db4}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:org41b1911}
...
\section{Results}
\label{sec:orge94b23a}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org614e179}
\subsubsection{Setup}
\label{sec:org011c8f4}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org6a5e348]{before}, and
...
\label{sec:org2131b53}

\subsubsection{Based on:}
\label{sec:org7aa66f8}
...
------------------- added Fri 22.05 -----------------------
\subsubsection{MCMC}
\label{sec:org1460dfe}
\paragraph{Setup}
\label{sec:org28cc3bd}
Denote the Lorenz-96 system \ref{eqn:lorenz_X}, \ref{eqn:lorenz_Y} with parameters \(u = [F, h, c, b]\) as
...
with:
\begin{itemize}
\item \(y \in \R^{5K}\): \uline{moment function of last state or mean of moment function}?
\item \(u \in \R^4\): Parameter vector
\item \(J(\mathcal{M}[\cdot] z_0) : \R^4 \to \R^?\): Observational operator
\item \(\eta \sim \N{0}{\Sigma}\), where \(\Sigma = r^2 [\text{var}(f(\tilde{z}))]\)
\end{itemize}

\paragraph{What is actually going on?}
\label{sec:orge640f03}
...
\paragraph{Independently of the correctness of the simulation, there is a serious feasibility challenge}
\label{sec:orgc8e55e7}
...
use \href{https://numpy.org/doc/1.18/reference/generated/numpy.roll.html?highlight=roll\#numpy.roll}{\texttt{np.roll}} and no explicit loops, so I'm not confident that they can be sped up significantly.
Maybe trying out something like \url{http://numba.pydata.org/} could give some speedup, I have no
