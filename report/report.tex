...
the standard random walk (see also \hyperref[sec:org684b1d3]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:orgd170901}

Works together with the \hyperref[sec:org82bf453]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:org3ca3368}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:org3899e62}
...
\section{Results}
\label{sec:orgfe154ed}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org6668244}
\subsubsection{Setup}
\label{sec:orge767d70}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org1a32010]{before}, and
...
\label{sec:orgfd7c759}
\subsubsection{Model}
\label{sec:orge85ce1d}
\paragraph{Based on:}
\label{sec:org4fb3586}
...
\item Due to the ergodic properties of \(\mathcal{M}\) \ref{sec:org16694ac} , it doesn't really matter what \(z_0\) is
...
\paragraph{Result}
\label{sec:org722fbe4}

\subparagraph{Density plot for posterior}
\label{sec:org4aca651}
...
\label{sec:org7b5273b}
\subsubsection{Model}
\label{sec:orgc4b0c84}
\paragraph{Burgers' Equation}
\label{sec:org3cd68e9}
...
\label{sec:org96f8d88}

Blabla
\subsubsection{MCMC}
\label{sec:org65697ae}

\paragraph{Hyperparameter tuning}
\label{sec:orgcc5d976}
...
\url{https://ethz.ch/content/dam/ethz/special-interest/baug/ifb/ifb-dam/homepage-IfB/Education/msc\_courses/msc\_computational-statphys/documents/2018\_CP-Lecture-Part1.pdf},
...
\subparagraph{Noise}
\label{sec:org3ef8263}

\(\eta \sim \N{0}{\gamma^2 \I_5}\) with \(\gamma = 0.05\).

\subparagraph{Prior}
\label{sec:org21dfffc}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/burgers_RW_n=5000_d=0.045_chain_report.png}
\caption{\label{fig:burgers_delta_045}
Evolution of the chain with a random walk proposal and \(\delta = 0.045\)}
\end{figure}

\subparagraph{Posterior estimates}
\label{sec:orgf6d3993}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/burgers_RW_n=5000_d=0.045_densities_report.png}
\caption{\label{fig:burgers_densities_delta_0045}
Posterior densities, taken from the RW-chain shown above with \(\delta = 0.045\), burn-in 500 and sampling interval 25.}
\end{figure}
