...
the standard random walk (see also \hyperref[sec:org7e4a39f]{Acceptance Probability for Metropolis-Hastings})
...
\label{sec:org0d5ee5f}

Works together with the \hyperref[sec:org0270c92]{pCNProposer} to achieve the simpler expression for the acceptance
$$a = \text{min}\{1, \exp(\Phi(u) - \Phi(v))\}.$$

\subsubsection{CountedAccepter}
\label{sec:orgd49f4fa}

Stores and forwards calls to an "actual" accepter. Counts calls and accepts and is used for
calculating the acceptance ratio.

\subsection{Sampler}
\label{sec:orgf62be34}
...
\section{Results}
\label{sec:orgc25e625}
\subsection{Analytic sampling from a bimodal Gaussian}
\label{sec:org9ff7ca3}
\subsubsection{Setup}
\label{sec:orgb675898}
...
I ended up using a normal \(\N{0}{1}\) as a prior and the potential described \hyperref[sec:org949f2a6]{before}, and
...
\label{sec:org07c5ca3}
\subsubsection{Model}
\label{sec:org9ce3e9f}
\paragraph{Based on:}
\label{sec:orgb72a4e1}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/acF_K=6_J=4_T=20_r=0.5.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{./figures/F_K=6_J=4_T=20_r=0.5.png}
